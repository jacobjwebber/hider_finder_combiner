...
import math
import stargan_vc.module as md
from common_nn import LinearNorm

class Generator1(nn.Module):
    # 1D convolutional architecture
    def __init__(self, in_ch, n_spk, z_ch, mid_ch, s_ch, f0_ch, normtype='IN', src_conditioning=False, pretrained_embedding=True, f0_conditioning=False):
        ...
        self.le6 = md.DeconvGLU1D(z_ch+s_ch+f0_ch, mid_ch, 5, 1, normtype)
        self.le7 = md.DeconvGLU1D(mid_ch+s_ch+f0_ch, mid_ch, 5, 1, normtype)
        self.le8 = md.DeconvGLU1D(mid_ch+s_ch+f0_ch, mid_ch, 8, 2, normtype)
        self.le9 = md.DeconvGLU1D(mid_ch+s_ch+f0_ch, mid_ch, 8, 2, normtype)
        self.le10 = nn.Conv1d(mid_ch+s_ch+f0_ch, in_ch, 9, stride=1, padding=(9-1)//2)
        ...
        if f0_conditioning:
            self.eb2 = nn.embedding(100, f0_ch)
